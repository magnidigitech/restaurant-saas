...
import { WORLDWIDE_TIMEZONES, TimezoneOption } from "@/core/constants/locales";
    ...
    <div className={`relative w-full ${className}`} ref={dropdownRef}>
      ...
      >
        <div className="flex items-center gap-2 truncate pr-2">
          <span className="font-mono text-[11px] opacity-60">{selectedOption.gmt}</span>
          <span className="truncate">{selectedOption.label}</span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[10px] opacity-70 font-mono">{selectedOption.time}</span>
          <svg
      ...
      {isOpen && (
        ...
        >
          ...
          {/* LIST OF TIMEZONES */}
          <div className="max-h-64 overflow-y-auto p-1.5 space-y-3 custom-scrollbar">
            {groupedOptions.size === 0 ? (
              <div className="py-6 text-center text-xs opacity-50">No matching timezones found</div>
            ) : (
              Array.from(groupedOptions.entries()).map(([region, options]) => (
                <div key={region} className="space-y-1">
                  {/* REGION HEADER */}
                  <div className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${isDark ? "text-[#8F95A3]" : "text-slate-400"}`}>
                    {region}
                  </div>
                  ...
                  {options.map((tz) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full px-2.5 py-2 rounded-xl text-xs font-medium flex items-center justify-between transition cursor-pointer ${
                          isSelected
                      ...
                      >
                        <div className="flex items-center gap-2 truncate pr-2">
                          <span className={`font-mono text-[10px] shrink-0 ${isSelected ? "text-blue-100" : "opacity-50"}`}>
                            {tz.gmt}
                          </span>
                          <span className="truncate">{tz.label}</span>
                        </div>
                        <span className={`font-mono text-[11px] shrink-0 ${isSelected ? "text-blue-100" : "opacity-60"}`}>
                          {tz.time}
                        </span>
                      </button>
                    );
