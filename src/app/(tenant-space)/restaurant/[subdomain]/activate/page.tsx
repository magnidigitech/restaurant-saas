...
function ActivateAccountContent() {
  ...
  const routeSubdomain = (params?.subdomain as string) || searchParams.get("subdomain") || "";
  const paramEmail = searchParams.get("email") || "";

  const [email, setEmail] = useState(paramEmail);
  const [restaurantName, setRestaurantName] = useState("");
  const [subdomain, setSubdomain] = useState(routeSubdomain);
  ...
  useEffect(() => {
    if (!token) {
      setVerifying(false);
      setTokenError("Missing invitation token. Please check your invitation link.");
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setFormError("Missing or invalid invitation token.");
      return;
    }
  ...
  if (tokenError) {
    return (
      <div className="relative w-full max-w-[440px] rounded-3xl bg-[#0D121D]/90 backdrop-blur-2xl border border-white/[0.08] p-7 sm:p-9 shadow-2xl space-y-6 text-center">
        ...
        <div className="pt-2">
          <Link
            href={`/restaurant/${subdomain || routeSubdomain}/login`}
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-white truncate text-xs">
              {restaurantName || `${(subdomain || routeSubdomain).toUpperCase()} Restaurant`}
            </p>
            <p className="text-[11px] font-mono text-amber-400 truncate">
              {subdomain || routeSubdomain}.restobird.com
            </p>
          </div>
        </div>
